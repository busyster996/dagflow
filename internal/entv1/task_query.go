...
func (_q *TaskQuery) QuerySteps() *StepQuery {
...
func (_q *TaskQuery) QueryParams() *ParamQuery {
...
func (_q *TaskQuery) QueryTaskSteps() *TaskStepQuery {
...
func (_q *TaskQuery) QueryTaskParams() *TaskParamQuery {
...
func (_q *TaskQuery) WithSteps(opts ...func(*StepQuery)) *TaskQuery {
	query := (&StepClient{config: _q.config}).Query()
...
func (_q *TaskQuery) WithParams(opts ...func(*ParamQuery)) *TaskQuery {
	query := (&ParamClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *TaskQuery) WithTaskSteps(opts ...func(*TaskStepQuery)) *TaskQuery {
	query := (&TaskStepClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *TaskQuery) WithTaskParams(opts ...func(*TaskParamQuery)) *TaskQuery {
	query := (&TaskParamClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *TaskQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Task, error) {
	...
	if query := _q.withSteps; query != nil {
		if err := _q.loadSteps(ctx, query, nodes,
			func(n *Task) { n.Edges.Steps = []*Step{} },
			func(n *Task, e *Step) { n.Edges.Steps = append(n.Edges.Steps, e) }); err != nil {
			return nil, err
		}
	}
	if query := _q.withParams; query != nil {
		if err := _q.loadParams(ctx, query, nodes,
			func(n *Task) { n.Edges.Params = []*Param{} },
			func(n *Task, e *Param) { n.Edges.Params = append(n.Edges.Params, e) }); err != nil {
			return nil, err
		}
	}
	if query := _q.withTaskSteps; query != nil {
		if err := _q.loadTaskSteps(ctx, query, nodes,
			func(n *Task) { n.Edges.TaskSteps = []*TaskStep{} },
			func(n *Task, e *TaskStep) { n.Edges.TaskSteps = append(n.Edges.TaskSteps, e) }); err != nil {
			return nil, err
		}
	}
	if query := _q.withTaskParams; query != nil {
		if err := _q.loadTaskParams(ctx, query, nodes,
			func(n *Task) { n.Edges.TaskParams = []*TaskParam{} },
			func(n *Task, e *TaskParam) { n.Edges.TaskParams = append(n.Edges.TaskParams, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (_q *TaskQuery) loadSteps(ctx context.Context, query *StepQuery, nodes []*Task, init func(*Task), assign func(*Task, *Step)) error {
	...
	query.Where(func(s *sql.Selector) {
		joinT := sql.Table(task.StepsTable)
		s.Join(joinT).On(s.C(step.FieldID), joinT.C(task.StepsPrimaryKey[1]))
		s.Where(sql.InValues(joinT.C(task.StepsPrimaryKey[0]), edgeIDs...))
		columns := s.SelectedColumns()
		s.Select(joinT.C(task.StepsPrimaryKey[0]))
...
func (_q *TaskQuery) loadParams(ctx context.Context, query *ParamQuery, nodes []*Task, init func(*Task), assign func(*Task, *Param)) error {
	edgeIDs := make([]driver.Value, len(nodes))
	byID := make(map[uint64]*Task)
	nids := make(map[uint64]map[*Task]struct{})
	for i, node := range nodes {
	...
	query.Where(func(s *sql.Selector) {
		joinT := sql.Table(task.ParamsTable)
		s.Join(joinT).On(s.C(param.FieldID), joinT.C(task.ParamsPrimaryKey[1]))
		s.Where(sql.InValues(joinT.C(task.ParamsPrimaryKey[0]), edgeIDs...))
		columns := s.SelectedColumns()
		s.Select(joinT.C(task.ParamsPrimaryKey[0]))
		s.AppendSelect(columns...)
		s.SetDistinct(false)
	})
	if err := query.prepareQuery(ctx); err != nil {
...
func (_q *TaskQuery) loadTaskSteps(ctx context.Context, query *TaskStepQuery, nodes []*Task, init func(*Task), assign func(*Task, *TaskStep)) error {
	...
	query.Where(predicate.TaskStep(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(task.TaskStepsColumn), fks...))
	}))
...
func (_q *TaskQuery) loadTaskParams(ctx context.Context, query *TaskParamQuery, nodes []*Task, init func(*Task), assign func(*Task, *TaskParam)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[uint64]*Task)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(taskparam.FieldTaskID)
	}
	query.Where(predicate.TaskParam(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(task.TaskParamsColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
