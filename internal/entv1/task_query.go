...
func (_q *TaskQuery) QueryExecutions() *ExecutionQuery {
...
func (_q *TaskQuery) WithExecutions(opts ...func(*ExecutionQuery)) *TaskQuery {
	query := (&ExecutionClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *TaskQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Task, error) {
	...
	if query := _q.withExecutions; query != nil {
		if err := _q.loadExecutions(ctx, query, nodes,
			func(n *Task) { n.Edges.Executions = []*Execution{} },
			func(n *Task, e *Execution) { n.Edges.Executions = append(n.Edges.Executions, e) }); err != nil {
			return nil, err
		}
	}
...
func (_q *TaskQuery) loadExecutions(ctx context.Context, query *ExecutionQuery, nodes []*Task, init func(*Task), assign func(*Task, *Execution)) error {
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(execution.FieldTaskID)
	}
	query.Where(predicate.Execution(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(task.ExecutionsColumn), fks...))
	}))
