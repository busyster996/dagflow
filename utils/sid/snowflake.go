...
const (
	...
	epochUnix     = 1730390400000
	timestampBits = 42
	kindBits      = 4
	nodeBits      = 6
	seqBits       = 11

	maxKindID   = 1<<kindBits - 1
	...
	nodeShift = seqBits
	maxID     = (1 << (timestampBits + kindBits + nodeBits + seqBits)) - 1
)
...
func NewSnowflake(kindID, nodeID int64) (*Snowflake, error) {
...
func (f ID) Time() time.Time { secs := int64(f) >> timeShift; return time.Unix(secs+epochUnix, 0) }
